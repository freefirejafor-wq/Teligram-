...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚡ WinGo 1Min Signal", callback_data="1min_menu")],
    ])

def wingo_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔔 Auto Signal ON", callback_data="wingo_on"),
         InlineKeyboardButton("🔕 Auto Signal OFF", callback_data="wingo_off")],
        [InlineKeyboardButton("📊 এখনই Signal দেখুন", callback_data="wingo_now")],
        [InlineKeyboardButton("🔙 Back", callback_data="back_main")],
    ])

def min1_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔔 1Min Auto ON", callback_data="1min_on"),
         InlineKeyboardButton("🔕 1Min Auto OFF", callback_data="1min_off")],
        [InlineKeyboardButton("📊 এখনই 1Min Signal", callback_data="1min_now")],
        [InlineKeyboardButton("🔙 Back", callback_data="back_main")],
    ])
...
async def cmd_start(u, c):
    ...
    await safe_reply(u.message,
        f"╔══════════════════════════╗\n"
        f"  🎯  *WinGo AI Signal Bot*\n"
        f"╚══════════════════════════╝\n\n"
        f"👋 স্বাগতম *{name}*!\n\n"
        f"🔹 WinGo 30s — ULTIMATE AI\n"
        f"🔹 WinGo 1Min — ULTIMATE AI\n\n"
        f"👇 নিচে ক্লিক করুন:",
...
async def cmd_test(u, c):
    ...
    hist = await loop.run_in_executor(executor, _w_fetch)
    if not hist:
        await safe_reply(u.message, "❌ ডেটা পাওয়া যায়নি।")
        return
    pred = _ultimate_predict(hist)
    size = pred["size"]
    conf = pred["confidence"]
...
async def handle_cb(u, c):
    ...
    if data == "back_main":
        await safe_edit(q.message,
            "👇 মেনু:", reply_markup=main_menu())

    elif data == "wingo_menu":
        await safe_edit(q.message,
            "🎯 *WinGo 30s Signal*\n\nনিচে ক্লিক করুন:",
            parse_mode="Markdown", reply_markup=wingo_menu())
        ...
        await safe_edit(q.message,
            "⚡ *WinGo 1Min Signal*\n\nনিচে ক্লিক করুন:",
            parse_mode="Markdown", reply_markup=min1_menu())

    elif data == "wingo_on":
        _wingo_subs.add(uid)
        await safe_edit(q.message,
            "✅ *WinGo 30s Auto Signal চালু!*\n\nপ্রতি 30s এ signal আসবে।",
            parse_mode="Markdown", reply_markup=wingo_menu())

    elif data == "wingo_off":
        _wingo_subs.discard(uid)
        await safe_edit(q.message,
            "🔕 *WinGo 30s Auto Signal বন্ধ।*",
            parse_mode="Markdown", reply_markup=wingo_menu())

    elif data == "1min_on":
        _1min_subs.add(uid)
        await safe_edit(q.message,
            "✅ *WinGo 1Min Auto Signal চালু!*",
            parse_mode="Markdown", reply_markup=min1_menu())

    elif data == "1min_off":
        _1min_subs.discard(uid)
        await safe_edit(q.message,
            "🔕 *WinGo 1Min Auto Signal বন্ধ।*",
            parse_mode="Markdown", reply_markup=min1_menu())
        ...
        hist = await loop.run_in_executor(executor, _w_fetch)
        if not hist:
            await safe_edit(q.message, "❌ ডেটা পাওয়া যায়নি।", reply_markup=wingo_menu())
            return
        pred = _ultimate_predict(hist)
        size = pred["size"]
        conf = pred["confidence"]
        ...
        bar_f = round(conf / 10)
        conf_bar = "█" * bar_f + "░" * (10 - bar_f)
        next_period = str(int(hist[0]["period"]) + 1) if hist[0]["period"].isdigit() else "পরবর্তী"
        await safe_edit(q.message,
            f"╔══════════════════════════╗\n"
            f"  🎯  *WinGo 30s — ULTIMATE*\n"
            f"╚══════════════════════════╝\n\n"
            f"📡 *PERIOD* `{str(next_period)[-8:]}` 🔴 LIVE\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"  {size_icon}  *{size}*  {color_icon}  *{color_name}*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🔢 *{num1}* বা *{num2}*\n"
            f"📊 `{conf_bar}` *{conf}%*\n\n"
            f"⏳ _30s পরে result আসবে_",
            parse_mode="Markdown", reply_markup=wingo_menu())
        ...
        if _1min_history:
            ...
            await safe_edit(q.message,
                "⏳ ডেটা লোড হচ্ছে, একটু অপেক্ষা করুন...",
                reply_markup=min1_menu())
